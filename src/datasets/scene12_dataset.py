...
class Scene12Dataset(Dataset):
    def __init__(self, configs, split):
        super(Scene12Dataset, self).__init__()
    ...
    def __getitem__(self, index):
        scene, room = self.data[index][:2]
        id1, id2, score = self.data[index][2:]
        
        frame1 = "frame-{:0>6}.".format(id1)
        frame2 = "frame-{:0>6}.".format(id2)
        img1_path = os.path.join(self.data_dir, scene, room, 'data', frame1+'color.jpg')
        img2_path = os.path.join(self.data_dir, scene, room, 'data', frame2+'color.jpg')
        pose1_path = os.path.join(self.data_dir, scene, room, 'data', frame1+'pose.txt')
        pose2_path = os.path.join(self.data_dir, scene, room, 'data', frame2+'pose.txt')
